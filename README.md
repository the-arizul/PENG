...
- **Mandatory Content Text Rendering Directive (Zero-Ghosting / No-False-Claim Rule):** Mandates across AGENTS.md and SKILL.md that whenever any option generating text output (such as verification guides, explanations, autopsy reports, or commit breakdowns) is selected, the agent MUST write and render the full content text on screen FIRST before calling the `ask_question` context menu modal.
